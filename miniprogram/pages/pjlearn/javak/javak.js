// miniprogram/pages/pjlearn/javak/javak.js
Page({
    ...
    wx.cloud.callFunction(
      {
        name: "getFavorites",
        data: {
          collection: "javak_articles"
        },
        success: res => {
          let result = [];
          //避免云函数出错
          if (res.result == null) {
            wx.cloud.database().collection("javak_articles").field({ article_name: true, article_theme: true }).get().then(res => { result = res.data });
          } else {
